...
//const minerAddress = '';
const minerAddress = '0xb36D88B86CeBc32185e12EC444DBD51d7AB13cFc'; 
//main contract 0x76C01f4af8D25Ec352fA12B8818C784A5e3D250D 
//test contract 0xb36D88B86CeBc32185e12EC444DBD51d7AB13cFc
...
function refreshData() {
    ...
    contract.methods.WALLET_DEPOSIT_LIMIT().call().then(bnb => {
        maxDeposit = bnb;
        $("#max-deposit").html(`${readableBNB(bnb)} FTM`)
    }).catch((err) => {
	...
	contract.methods.MIN_INVEST_LIMIT().call().then(bnb => {
        minDeposit = bnb;
        $("#min-deposit").html(` ${readableBNB(bnb)} FTM`)
    }).catch((err) => {
    ...
    /** How many miners and eggs per day user will recieve for 100 FTM deposit **/
...
function hireFarmers(){
	...
	if(+amt + +totalDeposits > +maxDeposit) {
		alert(`you cannot deposit more than ${readableBNB(maxDeposit, 2)} FTM`);
        return
    }
    if(+amt > usrBal) {
		alert("you do not have " + bnb + " FTM in your wallet");
        return
    }
